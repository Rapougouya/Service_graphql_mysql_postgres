...
from sqlalchemy import create_engine, text  # ← AJOUTER text ici
...
class DatabaseClient:
    ...
    def create_tables(self):
        from src.database.models import Base  # Import local pour éviter les circular imports
        try:
    ...
    def health_check(self):
        """Vérification de santé de la base de données - CORRIGÉE"""
        try:
            with self.get_session() as session:
                # ✅ CORRECTION : Utiliser text() pour SQLAlchemy 2.0
...
class MySQLClient(DatabaseClient):
    def __init__(self):
        ...
        super().__init__(settings.mysql_url)

class PostgreSQLClient(DatabaseClient):
    def __init__(self):
        from src.config.settings import settings
        super().__init__(settings.postgres_url)
